...
void getLambda( const int& N, double *L) { // äëèíà ñâîáîäíîãî ïðîáåãà íåéòðîíà
	for (int i = 0; i < N; i++) {
		double r = get_rand();
		L[i] = -log(r);
	}
}
int collisionProbability() { //Âåðîÿòíîñòü ðîæäåíèÿ N âòîðè÷íûõ íåéòðîíîâ ïîñëå ñòîëêíîâåíèÿ
...
struct Neutron {
	...
	double f;
	double vt;
	double vf; //ïðåäñòàâëåíèå êîîðäèíàò-èìïóëüñîâ â ñôåðè÷åñêèõ êîîðäèíàòàõ
};
struct NeutronDec { //ïðåäñòàâëåíèå â äåêàðòîâûõ êîîðäèíàòàõ
	double x, y, z;
	double vx, vy, vz;
};
void getVdirection(double *vt, double *vf) { //îïðåäåëåíèå íàïðàâëåíèÿ è ñêîðîñòè äâèæåíèÿ âòîðè÷íîãî íåéòðîíà
...
void collision(struct Neutron *P, struct Neutron *P2, int *N, int *b) { //îáðàáîòêà ñòîëêíîâåíèÿ íåéòðîíà ñ ÿäðîì
	int curN = 0;
	for (int i = 0; i < *N; i++) {
...
void shift(struct Neutron *P, int N, double *L) { //äâèæåíèå íåéòðîíîâ
...
int main(int argc, char** argv) {
	...
	for (int j = 0; j < 10000; j++) {
		...
		for (int i = 0; i < 20; i++) {
			...
			getLambda(N, L); //ïîëó÷åíèå äëèíû ñâîáîäíîãî ïðîáåãà
			shift(P, N, L); //ñìåùåíèå íåéòðîíîâ
			int * NewN = cleanCount(P, &N, &count); //èñêëþ÷åíèå íåéòðîíîâ , óëåòåâøèõ èç îáëàñòè ÿäåðíîé ðåàêöèè
			P = clean(P, &N, count, NewN);
			int *b = (int *)malloc(N * sizeof(int));
			int N1 = N;
			getb(b, &N1);
			struct Neutron *P2 = (struct Neutron *)malloc(N1 * sizeof(struct Neutron));
			collision(P, P2, &N, b); //îáðàáîòêà ñòîëêíîâåíèé íåéòðîíîâ â îáëàñòè ÿäåðíîé ðåàêöèè ñ ÿäðàìè
